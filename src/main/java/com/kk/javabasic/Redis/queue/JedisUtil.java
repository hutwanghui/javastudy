...
public class JedisUtil {
    ...
    static {
        ...
        File file = new File("jedis.properties");
        try {
            properties.load(new FileInputStream(file));
        } catch (IOException e) {
            e.printStackTrace();
        }
        JEDIS_IP = properties.getProperty("jedis.ip", "193.112.163.175");
        JEDIS_PORT = Integer.valueOf(properties.getProperty("jedis.port"));
        JEDIS_PASSWORD = properties.getProperty("jedis.password", "hadoop@spark");
        JedisPoolConfig config = new JedisPoolConfig();
        //config.setMaxActive(5000);
        //最大空闲连接数
        config.setMaxIdle(Integer.valueOf(properties.getProperty("jedis.pool.maxIdle", String.valueOf(10))));
        config.setMaxWaitMillis(5000L);
        //最大连接数
        config.setMaxTotal(Integer.valueOf(properties.getProperty("jedis.pool.maxActive", String.valueOf(200))));
    ...
    public static Jedis getJedis(){
        //pool.getResource()获得一个Jedis实例。
        //然后利用这个Jedis实例向Redisserver发送相关的指令操作，
        //最后调用Jedis类的close方法。将这个Jedis实例归还给JedisPool。
